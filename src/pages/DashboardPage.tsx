...
export default function DashboardPage() {
  ...
  const refreshGoal = useCallback(() => setGoal(getUserGoal()), [])

  // 페이지가 보여질 때마다 (라우트 복귀 포함) 목표 갱신
  useEffect(() => {
    refreshGoal()
    ...
    <div className="min-h-screen bg-[#0a0a0f] text-white">
      <header className="border-b border-white/10 px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={handleSignOut} className="text-sm text-gray-500 hover:text-white transition-colors">로그아웃</button>
        </div>
      </header>
      ...
      <main className="max-w-4xl mx-auto px-6 py-10">
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-8">
          ...
          {goal ? (
            <div className="bg-white/5 border border-white/10 rounded-2xl px-6 py-4 flex items-center gap-6 min-w-[280px]">
              ...
              <Link to="/goal-setup" className="ml-auto text-gray-600 hover:text-white text-xs border border-white/10 px-2 py-1 rounded-lg transition-colors">
                수정
              </Link>
            </div>
          ) : (
        ...
        {!goal?.currentLevel && (
          <Link to="/level-test"
            ...
            <span className="text-purple-400 text-sm font-semibold">시작 →</span>
          </Link>
        )}
        ...
        <div className="grid md:grid-cols-2 gap-4 mb-8">
          <Link to="/practice"
            ...
            <p className="text-gray-400 text-sm mb-6">개인 배경, 서베이 주제, 롤플레이, 돌발 질문<br />문제 음성 듣기 + AI 채점</p>
          ...
          <Link to="/mock-exam"
            ...
            <p className="text-gray-400 text-sm mb-6">실제 OPIC과 유사한 방식으로<br />4개 문제 연속 + 종합 채점</p>
